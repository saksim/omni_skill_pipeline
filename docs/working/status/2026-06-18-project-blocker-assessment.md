...
- [../../latest/architecture/agent-skill-package-model.md](../../latest/architecture/agent-skill-package-model.md)
- [../../latest/architecture/portable-skill-renderer.md](../../latest/architecture/portable-skill-renderer.md)
...
本评估支持这个拆分，并应归入 `docs/working/status/`。

原因：

- 旧 `docs/current/`/`docs/history/` 结构曾混合当前事实、历史 baseline、发布证据、长任务日志和产品上线计划。
- 这会放大 `GO`/`HOLD` 的理解成本。
- 也会让“唯一真实下一步”变得不清晰。

后续维护时，应避免再把旧 `docs/current/` 路径作为当前入口。
