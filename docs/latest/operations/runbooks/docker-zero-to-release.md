...
汇总包输出为 `release-artifacts-<RELEASE_ID>.tar.gz`。把该包交给评审者即可获得完整日志、每阶段 exit code、`docs/working/status/baselines` 证据、coverage XML、`summary.tsv` 和 `summary.json`。
...
```bash
docker run --rm \
  -v "$PWD/docs/working/status/baselines:/app/docs/working/status/baselines" \
  omni-skill-pipeline:test \
  python scripts/ci.py --python python3 --keep-going --isolate-test-files --coverage-fail-under 50 --coverage-xml docs/working/status/baselines/coverage.xml
```
...
- 产物优先看 `docs/working/status/baselines/coverage.xml` 与终端 summary。
...
- distill happy path 样例按 `docs/latest/operations/api.md` 分 text/audio/image/tabular/video 执行。
