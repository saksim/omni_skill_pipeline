...
| `scripts/gl64_real_loop_manifest_preflight.py` | Validate dropped real-loop manifests before launch evidence ingestion. | After operators add sanitized real-loop manifests and before GL-13. | GL-63 workpack, manifest directory. | Preflight JSON report and summary. | `python scripts/gl64_real_loop_manifest_preflight.py --fail-on-invalid --fail-on-pending --print-json` | Missing=0, invalid=0, all slot manifests have real source trace, review trace, source hash, redaction status, and passed agent smoke. | Missing slot file, fixture/simulated evidence, unreadable evidence refs, invalid hashes, `not_run` agent smoke. | real-loop / launch |
