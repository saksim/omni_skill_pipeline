...
- Environment variables are documented in `docs/latest/operations/env.md`; configuration is not hidden only in source.
...
- Has `scripts/ci.py` and `scripts/tp_tests.py` (TP=Task Package)
...
- API and environment documentation are present in `docs/latest/operations/`
