...
- Required artifacts:
  ...
  - `docs/working/status/baselines/e11-quality-regression-manifest.json`
  - `docs/working/status/baselines/e11-perf-cost-baseline-manifest.json`
  - `docs/working/status/baselines/e13-postgres-soak-plan.json`
  - `docs/working/status/baselines/e13-postgres-soak-benchmark-report.json`
...
- Decision records must be written to history snapshot docs under `docs/archive/status/`.
...
```bash
python scripts/release_switch.py --python python3 --postgres-dsn "$OMNI_TEST_POSTGRES_DSN" --output docs/working/status/baselines/e13-release-switch-validation-plan.json --decision-output docs/working/status/baselines/e13-release-switch-decision-report.json
...
python scripts/pg_soak.py --python python3 --postgres-dsn "$OMNI_TEST_POSTGRES_DSN"
```
