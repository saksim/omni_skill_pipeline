...
def _gl59_report(*, status: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    open_count = sum(
    ...
    return {
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_counts": {
            ...
            "cadence_stall_cycle_count_gl58": 0,
            "cadence_overdue_stalled_cycles_threshold_gl58": 2,
            "escalate_after_due_hours": 24.0,
        },
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_rows": rows,
    }


def _gl57_report(*, rows: list[dict[str, Any]], previous_report_path: str = "") -> dict[str, Any]:
    open_count = sum(
    ...
    return {
        ...
        "generated_at_utc": "2026-06-01T00:00:00Z",
        "input_paths": {
            "previous_submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_report": previous_report_path
        },
...
class RealTrialSubmissionQueueFollowupResolutionEscalationActionPlanClosureCadenceEscalationAcknowledgementClosureCadenceEscalationClosureCadenceEscalationClosureCadenceEscalationClosureScriptTests(
    unittest.TestCase
):
    def test_not_required_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            _write_json(
                gl59,
                _gl59_report(
                    status="ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_NOT_REQUIRED",
                    rows=[],
                ),
            )
            _write_json(gl57, _gl57_report(rows=[]))
            ...
            payload = json.loads(gl60.read_text(encoding="utf-8"))
            self.assertEqual(
                payload.get(
    ...
    def test_progressing_path_reports_gl57_backed_net_new_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            gl59 = root / "gl59.json"
            gl57 = root / "gl57.json"
            previous_gl57 = root / "previous-gl57.json"
            previous_gl60 = root / "previous-gl60.json"
            gl60 = root / "gl60.json"

            _write_json(
                gl59,
                _gl59_report(
                    status="ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_MONITORING",
                    rows=[
                        {
                            "escalation_item_id": "gl59-esc-002",
                            "escalation_item_status": "open",
                            "escalation_severity": "monitor",
                            "action_id_gl48": "gl46-slot-002-audio",
                            "required_modality_gl47": "audio",
                            "closure_item_id_gl57": "gl57-closure-cadence-escalation-closure-gl56-esc-002",
                            "closure_progress_state_gl57": "carried_open",
                            "closure_item_status_gl57": "open",
                            "escalation_item_id_gl56": "gl56-esc-002",
                            "acknowledgement_ingestion_item_id_gl55": "gl55-ingestion-002",
                            "cadence_item_status_gl58": "baseline_open",
            ...
            _write_json(
                ...
                _gl57_report(
                    ...
                    previous_report_path=str(previous_gl57),
                ),
            )
            _write_json(
                previous_gl60,
                {
                    ...
                    "generated_at_utc": "2026-06-01T00:30:00Z",
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_rows": [
                        ...
                        {
                            "closure_item_status": "open",
                            "escalation_item_id_gl59": "gl59-esc-002",
                            "action_id_gl48": "gl46-slot-002-audio",
                        },
                    ],
                },
            ...
            self.assertEqual(
                int(counts.get("net_new_closed_backed_by_gl57_net_new_closed_item_count", 0)),
                1,
            )
            self.assertEqual(
                payload.get(
