    ...
    *,
    ...
    return {
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_counts": {
            ...
            "closed_item_count": len(rows) - open_count,
            "previous_open_item_count_gl50": open_count,
            ...
            "ack_loop_mismatch_open_item_count": 0,
            "ack_missing_open_item_count": 0,
            "missing_handoff_open_item_count": 0,
            "net_new_launch_gate_eligible_loop_count": int(net_new_launch_gate_eligible_loop_count),
            "open_item_count_delta": 0,
        },
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_rows": rows,
    }


class RealTrialSubmissionQueueFollowupResolutionEscalationActionPlanClosureCadenceEscalationAcknowledgementClosureCadenceScriptTests(
    unittest.TestCase
):
    def test_not_required_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            _write_json(
                gl51,
                _gl51_report(
                    status="ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_NOT_REQUIRED",
            ...
            payload = json.loads(gl52.read_text(encoding="utf-8"))
            self.assertEqual(
                payload.get(
    ...
    def test_due_path_from_previous_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            _write_json(
                gl51,
                _gl51_report(
                    status="ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_PROGRESSING",
                    generated_at_utc="2026-05-30T00:00:00Z",
                    rows=[
                        {
                            ...
                            "owner_gl50": "controlled-beta-ops",
            ...
            self.assertIn("acknowledgement_ingestion_closure_cadence_due", warning_codes)
            rows = payload.get(
    ...
    def test_overdue_stalled_fail_on_requested(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            _write_json(
                gl51,
                _gl51_report(
                    status="ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_STALLED",
                    generated_at_utc="2026-05-30T00:00:00Z",
                    rows=[
                        {
                            "acknowledgement_ingestion_item_id": "gl50-ack-ingestion-gl46-slot-001-text",
            ...
            warning_codes = payload.get("warning_codes", [])
            self.assertIn("acknowledgement_ingestion_closure_cadence_overdue_stalled", warning_codes)
            self.assertIn("acknowledgement_ingestion_closure_stalled", warning_codes)
