...
class TPRegistryScriptTests(unittest.TestCase):
    ...
    def test_all_dry_run_builds_unittest_command(self) -> None:
        ...
        self.assertIn("Command chunks:", completed.stdout)

    def test_dry_run_chunks_long_unittest_commands(self) -> None:
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/tp_tests.py",
        ...
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("Command chunks:", completed.stdout)
        self.assertGreater(completed.stdout.count("Command:"), 1)
        self.assertIn("Command chunk:", completed.stdout)
