...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-49 escalation diagnostics from GL-48 cadence reports by identifying "
            "due/overdue stalled cadence action rows and assigning escalation urgency."
        )
    )
    parser.add_argument(
        "--submission-queue-followup-resolution-escalation-action-plan-closure-cadence-report",
        default=str(DEFAULT_CADENCE_REPORT_PATH),
    )
    parser.add_argument(
        "--output",
        default=str(DEFAULT_OUTPUT_PATH),
        help='GL-49 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-49 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Exit with code 1 when status is ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_OVERDUE_STALLED.",
    )
...
def _status_from_inputs(
    *,
    ...
    status = cadence_status.strip().upper()
    if status == "ACTION_PLAN_CLOSURE_CADENCE_NOT_REQUIRED" and open_item_count <= 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_NOT_REQUIRED"
    if blocked_overdue_stalled_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_OVERDUE_STALLED"
    if escalation_due_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_DUE"
    if open_item_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_MONITORING"
    return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_CLEARED"


def _build_report(
    *,
    cadence_report: dict[str, Any],
    cadence_report_path: Path,
    owner: str,
    now_utc: datetime,
    escalate_after_due_hours: float,
) -> dict[str, Any]:
    cadence_status = str(
        cadence_report.get("followup_resolution_escalation_action_plan_closure_cadence_status", "unknown")
    ).strip()
    cadence_warning_codes = cadence_report.get("warning_codes", [])
    if not isinstance(cadence_warning_codes, list):
        cadence_warning_codes = []

    cadence_counts = cadence_report.get(
    ...
    cadence_refresh = cadence_report.get("refresh_cadence", {})
    if not isinstance(cadence_refresh, dict):
        cadence_refresh = {}

    cadence_rows = cadence_report.get(
    ...
    next_refresh_due_utc = str(cadence_refresh.get("next_refresh_due_utc", "")).strip()
    next_refresh_due_dt = _utc_timestamp_to_datetime(next_refresh_due_utc)
    ...
    for row in cadence_rows:
        ...
        closure_state_gl47 = str(row.get("closure_state_gl47", "")).strip().lower()
        ...
        row_owner = str(row.get("owner_gl47", "")).strip() or owner
        linked_submission_loop_id_gl47 = str(row.get("linked_submission_loop_id_gl47", "")).strip()

        if cadence_item_status in {"closed", "closed_since_previous_cycle"}:
            continue

        escalation_severity = "monitor"
        escalation_reason_code = "cadence_monitoring"
        escalation_action = "continue_cadence_monitoring"
        escalation_item_status = "open"

        if cadence_status.upper() == "ACTION_PLAN_CLOSURE_CADENCE_OVERDUE_STALLED":
            escalation_severity = "blocked_overdue_stalled"
            escalation_reason_code = "action_plan_closure_cadence_overdue_stalled"
            escalation_action = "escalate_operator_immediately"
            blocked_overdue_stalled_count += 1
        elif cadence_item_status == "refresh_due":
            due_hours = _to_float(cadence_refresh.get("due_in_hours", 0.0), default=0.0)
            overdue_by_hours = abs(due_hours) if due_hours < 0 else 0.0
            if overdue_by_hours >= escalate_after_due_hours:
                escalation_severity = "due_breached"
                escalation_reason_code = "action_plan_closure_cadence_due_breached"
                escalation_action = "escalate_due_breach"
                escalation_due_count += 1
            else:
                escalation_severity = "due"
                escalation_reason_code = "action_plan_closure_cadence_due"
                escalation_action = "execute_due_refresh_cycle"
                escalation_due_count += 1
        elif cadence_item_status == "baseline_open":
            escalation_severity = "baseline_open"
            escalation_reason_code = "action_plan_closure_cadence_baseline_initialized"
            escalation_action = "start_first_refresh_cycle"
        else:
            escalation_severity = "on_schedule"
            escalation_reason_code = "action_plan_closure_cadence_on_schedule"
            escalation_action = "monitor_until_due"

        escalation_rows.append(
            {
                "escalation_item_id": "gl49-cadence-escalation-%s" % (action_id or "unbound"),
                "escalation_item_status": escalation_item_status,
                ...
                "cadence_item_status_gl48": cadence_item_status,
                "required_modality_gl47": required_modality_gl47,
                "linked_submission_loop_id_gl47": linked_submission_loop_id_gl47,
                "next_refresh_due_utc_gl48": next_refresh_due_utc,
    ...
    if open_item_count > 0:
        warning_codes.append("open_action_plan_closure_cadence_escalation_items_present")
    if blocked_overdue_stalled_count > 0:
        warning_codes.append("action_plan_closure_cadence_overdue_stalled_escalation_required")
    if escalation_due_count > 0:
        warning_codes.append("action_plan_closure_cadence_due_escalation_required")
    ...
    counts = {
        ...
        "cadence_stall_cycle_count_gl48": _to_int(cadence_counts.get("stall_cycle_count", 0), default=0),
        "cadence_overdue_stalled_cycles_threshold_gl48": _to_int(
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_report": str(
                cadence_report_path
            ),
        },
        "followup_resolution_escalation_action_plan_closure_cadence_status_gl48": cadence_status,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_status": status,
        "warning_codes": warning_codes,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_counts": counts,
        "owner_followup_resolution_escalation_action_plan_closure_cadence_escalation_counts": owner_counts,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_rows": escalation_rows,
    }


def _render_summary(report: dict[str, Any]) -> str:
    counts = report.get("followup_resolution_escalation_action_plan_closure_cadence_escalation_counts", {})
    if not isinstance(counts, dict):
        counts = {}
    warning_codes = report.get("warning_codes", [])
    if not isinstance(warning_codes, list):
        warning_codes = []
    rows = report.get("followup_resolution_escalation_action_plan_closure_cadence_escalation_rows", [])
    if not isinstance(rows, list):
        rows = []

    lines = [
        "# Real Trial Submission Queue Follow-Up Resolution Escalation Action Plan Closure Cadence Escalations Summary",
        "",
        "- GL-48 cadence status: `%s`"
        % str(report.get("followup_resolution_escalation_action_plan_closure_cadence_status_gl48", "unknown")),
        "- GL-49 escalation status: `%s`"
        % str(
            report.get(
        ...
        for row in rows:
            ...
            lines.append(
                ...
                % (
                    ...
                    str(row.get("cadence_item_status_gl48", "")),
...
def main() -> int:
    args = _parse_args()
    cadence_report_path = Path(
        str(args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_report).strip()
    ).resolve()
    ...
    try:
        if not cadence_report_path.is_file():
            raise ValueError("GL-48 cadence report path does not exist: %s" % cadence_report_path)
        cadence_report = _read_json(cadence_report_path)
        ...
        report = _build_report(
            ...
            cadence_report_path=cadence_report_path,
            owner=owner,
            now_utc=now_utc,
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalations report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalations summary written: %s"
            % summary_path
        )
    ...
    print(
        "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalations status=%s open=%s overdue_stalled=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_status",
    ...
    if bool(args.fail_on_overdue_stalled) and str(
        report.get("followup_resolution_escalation_action_plan_closure_cadence_escalation_status", "")
    ).strip().upper() == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_OVERDUE_STALLED":
        return 1
    return 0
