...
DEFAULT_ACK_CLOSURE_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "current"
    / "status"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-52 acknowledgement-closure-cadence diagnostics by binding GL-51 "
            "acknowledgement-closure status to refresh SLA timing and stalled-cycle accumulation."
        )
    )
    parser.add_argument(
        "--submission-queue-followup-resolution-escalation-action-plan-closure-cadence-escalation-acknowledgement-closure-report",
        default=str(DEFAULT_ACK_CLOSURE_REPORT_PATH),
    )
    parser.add_argument(
        "--previous-acknowledgement-closure-cadence-report",
        default="",
        help=(
            "Optional previous GL-52 report. When omitted, script attempts to read existing "
    ...
    parser.add_argument(
        ...
        help='GL-52 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-52 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=24.0,
        help="Refresh cadence interval for GL-52 timing diagnostics (default: 24).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_OVERDUE_STALLED."
        ),
    )
...
def _build_report(
    *,
    acknowledgement_closure_report: dict[str, Any],
    acknowledgement_closure_report_path: Path,
    ...
    closure_counts = acknowledgement_closure_report.get(
    ...
    if not isinstance(closure_counts, dict):
        closure_counts = {}
    closure_rows = acknowledgement_closure_report.get(
    ...
    if not isinstance(closure_rows, list):
        closure_rows = []
    closure_status_gl51 = str(
        acknowledgement_closure_report.get(
            "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_status",
            "unknown",
        )
    ).strip()
    closure_warning_codes = acknowledgement_closure_report.get("warning_codes", [])
    ...
    open_item_count = _to_int(closure_counts.get("open_item_count", 0), default=0)
    total_item_count = _to_int(closure_counts.get("total_item_count", 0), default=0)
    stale_open_item_count = _to_int(closure_counts.get("stale_open_item_count", 0), default=0)
    net_new_closed_item_count_gl51 = _to_int(
        closure_counts.get("net_new_closed_item_count", 0),
        default=0,
    )
    net_new_launch_gate_eligible_loop_count_gl51 = _to_int(
    ...
    not_required = (
        open_item_count <= 0
        and closure_status_gl51.upper()
        in {
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_NOT_REQUIRED",
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CLEARED",
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_COMPLETE",
        }
    )

    if (
        closure_status_gl51.upper()
    ...
    if cadence_status == "CADENCE_NOT_REQUIRED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_NOT_REQUIRED"
    elif cadence_status == "CADENCE_BASELINE_INITIALIZED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_BASELINE_INITIALIZED"
    elif cadence_status == "CADENCE_ON_SCHEDULE":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ON_SCHEDULE"
    elif cadence_status == "CADENCE_OVERDUE_STALLED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_OVERDUE_STALLED"
    else:
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_DUE"
    ...
    if cadence_status == "CADENCE_DUE":
        warning_codes.append("acknowledgement_ingestion_closure_cadence_due")
    if cadence_status == "CADENCE_OVERDUE_STALLED":
        warning_codes.append("acknowledgement_ingestion_closure_cadence_overdue_stalled")
    if (
        closure_status_gl51.upper()
        == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_STALLED"
    ):
        warning_codes.append("acknowledgement_ingestion_closure_stalled")
    ...
    for row in closure_rows:
        if not isinstance(row, dict):
            continue
        closure_state_gl51 = str(row.get("closure_state", "")).strip()
        if closure_state_gl51 in {"open_new", "open_carried"}:
        ...
        elif closure_state_gl51 == "closed_since_previous_cycle":
        ...
        cadence_rows.append(
            {
                ...
                "closure_state_gl51": closure_state_gl51,
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_report": str(
                acknowledgement_closure_report_path
            ),
            "previous_acknowledgement_closure_cadence_report": str(previous_report_path)
            if previous_report_path is not None and previous_available
            else "",
        },
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_status_gl51": closure_status_gl51,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_status": top_level_status,
        "warning_codes": warning_codes,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_counts": {
            "total_item_count": total_item_count,
            "open_item_count": open_item_count,
            "stale_open_item_count": stale_open_item_count,
            "net_new_closed_item_count_gl51": net_new_closed_item_count_gl51,
            "net_new_launch_gate_eligible_loop_count_gl51": net_new_launch_gate_eligible_loop_count_gl51,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_rows": cadence_rows,
    }
...
def _render_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Real Trial Submission Queue Follow-Up Resolution Escalation Action Plan Closure Cadence Escalation Acknowledgement Closure Cadence Summary",
        "",
        "- GL-51 acknowledgement-closure status: `%s`"
        % str(
            report.get(
                "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_status_gl51",
                "unknown",
            )
        ),
        ...
        for row in rows:
            ...
            lines.append(
                ...
                % (
                    ...
                    str(row.get("closure_state_gl51", "")),
...
def main() -> int:
    ...
    acknowledgement_closure_report_path = Path(
        str(
            args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_report
        ).strip()
    ).resolve()
    ...
    try:
        ...
        if not acknowledgement_closure_report_path.is_file():
            raise ValueError("GL-51 acknowledgement-closure report path does not exist: %s" % acknowledgement_closure_report_path)
        acknowledgement_closure_report = _read_json(acknowledgement_closure_report_path)

        previous_report_path: Path | None = None
        previous_report: dict[str, Any] | None = None
        previous_arg = str(args.previous_acknowledgement_closure_cadence_report).strip()
        if previous_arg:
        ...
        report = _build_report(
            acknowledgement_closure_report=acknowledgement_closure_report,
            acknowledgement_closure_report_path=acknowledgement_closure_report_path,
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence summary written: %s"
            % summary_path
        )
    ...
    print(
        "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence status=%s open_items=%s stall_cycles=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_status",
    ...
    ).strip().upper() == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_OVERDUE_STALLED":
        return 1
    return 0
