...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-58 escalation-closure-cadence diagnostics by binding GL-57 "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional previous GL-58 report. When omitted, script attempts to read existing "
    ...
    parser.add_argument(
        ...
        help='GL-58 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-58 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=24.0,
        help="Refresh cadence interval for GL-58 timing diagnostics (default: 24).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_OVERDUE_STALLED."
        ),
    )
...
def _build_report(
    *,
    ...
    net_new_closed_item_count_gl57 = _to_int(
        escalation_closure_counts.get("net_new_closed_item_count", 0),
        default=0,
    )
    net_new_closed_backed_by_gl54_net_new_closed_item_count = _to_int(
        escalation_closure_counts.get("net_new_closed_backed_by_gl54_net_new_closed_item_count", 0),
        default=0,
    )

    not_required = (
        open_item_count <= 0
        and escalation_closure_status_gl57.upper()
        in {
            ...
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CLEARED",
        }
    )

    if (
        escalation_closure_status_gl57.upper()
    ...
    if cadence_status == "CADENCE_NOT_REQUIRED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_NOT_REQUIRED"
    elif cadence_status == "CADENCE_BASELINE_INITIALIZED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_BASELINE_INITIALIZED"
    elif cadence_status == "CADENCE_ON_SCHEDULE":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ON_SCHEDULE"
    elif cadence_status == "CADENCE_OVERDUE_STALLED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_OVERDUE_STALLED"
    else:
    ...
    if cadence_status == "CADENCE_DUE":
        warning_codes.append("acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_due")
    if cadence_status == "CADENCE_OVERDUE_STALLED":
        warning_codes.append("acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_overdue_stalled")
    if (
        escalation_closure_status_gl57.upper()
        == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_STALLED"
    ):
        warning_codes.append("acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_stalled")
    ...
    for row in escalation_closure_rows:
        if not isinstance(row, dict):
            continue
        closure_item_status_gl57 = str(row.get("closure_item_status", "")).strip().lower()
        if closure_item_status_gl57 != "open":
            continue

        closure_progress_state_gl57 = str(row.get("closure_progress_state", "")).strip().lower()
        if closure_progress_state_gl57 in {"carried_open", "net_new_open"}:
        ...
        elif closure_progress_state_gl57 == "closed_since_previous_cycle":
        ...
        cadence_rows.append(
            {
                "closure_item_id_gl57": str(row.get("closure_item_id", "")).strip(),
                "closure_progress_state_gl57": closure_progress_state_gl57,
                "closure_item_status_gl57": closure_item_status_gl57,
                "owner_gl57": str(row.get("owner", "")).strip() or owner,
                ...
                "escalation_severity_gl56": str(row.get("escalation_severity_gl56", "")).strip(),
                "cadence_item_status": cadence_item_status,
                "next_refresh_due_utc_gl58": _datetime_to_utc_iso(next_refresh_due_dt),
            }
        )
    ...
    return {
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_status": top_level_status,
        "warning_codes": warning_codes,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_counts": {
            "total_item_count": total_item_count,
            "open_item_count": open_item_count,
            "stale_open_item_count": stale_open_item_count,
            "net_new_closed_item_count_gl57": net_new_closed_item_count_gl57,
            "net_new_closed_backed_by_gl54_net_new_closed_item_count": net_new_closed_backed_by_gl54_net_new_closed_item_count,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_rows": cadence_rows,
    }
    ...
    if rows:
        for row in rows:
            ...
            lines.append(
                ...
                % (
                    str(row.get("closure_item_id_gl57", "")),
                    str(row.get("closure_progress_state_gl57", "")),
...
def main() -> int:
    ...
    try:
        ...
        if not escalation_closure_report_path.is_file():
            raise ValueError(
                "GL-57 escalation-closure report path does not exist: %s"
                % escalation_closure_report_path
            )
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence summary written: %s"
            % summary_path
        )
    ...
    print(
        "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence status=%s open_items=%s stall_cycles=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_status",
