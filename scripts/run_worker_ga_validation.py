...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Run or print a worker GA-hardening validation command pack for Linux.',
    )
    parser.add_argument(
        '--python',
        default='python3',
    ...
    parser.add_argument(
        ...
        help='Validation stages to include. Defaults to full worker GA command pack.',
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
...
def _build_stage_map(*, python_cmd: list[str]) -> dict[str, StageSpec]:
    return {
        'worker_corpus': StageSpec(
            name='worker_corpus',
            description='Validate corpus job dispatch path in local worker runtime.',
            command=[
                *python_cmd,
                '-m',
                'unittest',
                'tests.test_worker.WorkerCorpusIntegrationTests',
            ],
        ),
        'worker_retry': StageSpec(
            name='worker_retry',
            description='Validate transient retry vs permanent failure worker behavior.',
            command=[
                *python_cmd,
                '-m',
                'unittest',
                'tests.test_worker.WorkerRetryPolicyTests',
            ],
        ),
        'worker_idempotency': StageSpec(
            name='worker_idempotency',
            description='Validate idempotency-key and duplicate payload short-circuit behavior.',
            command=[
                *python_cmd,
                '-m',
                'unittest',
                'tests.test_worker.WorkerIdempotencyTests',
            ],
        ),
        'worker_claim_lock': StageSpec(
            name='worker_claim_lock',
            description='Validate multi-worker claim/lock safety on the same pending job.',
            command=[
                *python_cmd,
                '-m',
                'unittest',
                'tests.test_worker.WorkerConcurrencyClaimTests',
            ],
        ),
        'worker_task_types': StageSpec(
            name='worker_task_types',
            description='Validate upgraded worker task types (review/rebuild/revise) stay executable.',
            command=[
                *python_cmd,
                '-m',
                'unittest',
                'tests.test_worker.WorkerTaskTypeUpgradeTests',
            ],
        ),
    }
