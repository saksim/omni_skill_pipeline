...
DEFAULT_ACTION_PLAN_CLOSURE_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "working"
    / "status"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-48 cadence diagnostics by binding GL-47 action-plan-closure status "
            "to refresh-SLA timing and stalled-cycle accumulation."
        )
    )
    parser.add_argument(
        "--submission-queue-followup-resolution-escalation-action-plan-closure-report",
        default=str(DEFAULT_ACTION_PLAN_CLOSURE_REPORT_PATH),
    )
    parser.add_argument(
        "--collection-report",
    ...
    parser.add_argument(
        "--previous-action-plan-closure-cadence-report",
        default="",
        help=(
            "Optional previous GL-48 report. When omitted, script attempts to read existing "
    ...
    parser.add_argument(
        ...
        help='GL-48 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-48 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=24.0,
        help="Refresh cadence interval for GL-48 timing diagnostics (default: 24).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Exit with code 1 when cadence status is ACTION_PLAN_CLOSURE_CADENCE_OVERDUE_STALLED.",
    )
...
def _build_report(
    *,
    action_plan_closure_report: dict[str, Any],
    action_plan_closure_report_path: Path,
    ...
    open_action_count = _to_int(closure_counts.get("open_action_count", 0), default=0)
    total_action_count = _to_int(closure_counts.get("total_action_count", 0), default=0)
    stale_open_action_count = _to_int(closure_counts.get("stale_open_action_count", 0), default=0)
    net_new_closed_action_count_gl47 = _to_int(
        closure_counts.get("net_new_closed_action_count", 0),
        default=0,
    )
    net_new_launch_gate_eligible_loop_count_gl47 = _to_int(
        closure_counts.get("net_new_launch_gate_eligible_loop_count", 0),
        default=0,
    )

    not_required = (
        open_action_count <= 0
        and closure_status_gl47.upper()
        in {
            "ACTION_PLAN_CLOSURE_NOT_REQUIRED",
            "ACTION_PLAN_CLOSURE_CLEARED",
            "ACTION_PLAN_CLOSURE_COMPLETE",
        }
    )

    if closure_status_gl47.upper() == "ACTION_PLAN_CLOSURE_STALLED" and open_action_count > 0:
    ...
    if not_required:
        ...
        if now_utc >= next_refresh_due_dt:
            if (
                closure_status_gl47.upper() == "ACTION_PLAN_CLOSURE_STALLED"
    ...
    if cadence_status == "CADENCE_NOT_REQUIRED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_NOT_REQUIRED"
    elif cadence_status == "CADENCE_BASELINE_INITIALIZED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_BASELINE_INITIALIZED"
    elif cadence_status == "CADENCE_ON_SCHEDULE":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ON_SCHEDULE"
    elif cadence_status == "CADENCE_OVERDUE_STALLED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_OVERDUE_STALLED"
    else:
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_DUE"

    warning_codes: list[str] = []
    if open_action_count > 0:
        warning_codes.append("open_followup_resolution_escalation_action_plan_closure_items_present")
    if cadence_status == "CADENCE_DUE":
        warning_codes.append("action_plan_closure_cadence_due")
    if cadence_status == "CADENCE_OVERDUE_STALLED":
        warning_codes.append("action_plan_closure_cadence_overdue_stalled")
    if closure_status_gl47.upper() == "ACTION_PLAN_CLOSURE_STALLED":
        warning_codes.append("action_plan_closure_stalled")
    if open_action_count > 0 and net_new_launch_gate_eligible_loop_count_gl47 <= 0:
        warning_codes.append("no_net_new_launch_gate_eligible_real_loops")
    warning_codes.extend(str(item).strip() for item in closure_warning_codes if str(item).strip())
    ...
    for row in closure_rows:
        if not isinstance(row, dict):
            continue
        closure_state_gl47 = str(row.get("closure_state", "")).strip()
        if closure_state_gl47 in {"open_new", "open_carried"}:
        ...
        elif closure_state_gl47 == "closed_since_previous_cycle":
        ...
        cadence_rows.append(
            {
                ...
                "closure_state_gl47": closure_state_gl47,
                "cadence_item_status": cadence_item_status,
                "required_modality_gl47": str(row.get("required_modality_gl46", "")).strip().lower(),
                "owner_gl47": str(row.get("owner_gl46", "")).strip(),
                "linked_submission_loop_id_gl47": str(row.get("linked_submission_loop_id_gl46", "")).strip(),
            }
        )
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_report": str(
                action_plan_closure_report_path
            ),
            "collection_report": str(collection_report_path),
            "previous_action_plan_closure_cadence_report": str(previous_report_path)
            if previous_report_path is not None and previous_available
            else "",
        },
        "followup_resolution_escalation_action_plan_closure_status_gl47": closure_status_gl47,
        "collection_program_status_gl12": str(collection_alignment.get("program_status", "unknown")).strip(),
        "followup_resolution_escalation_action_plan_closure_cadence_status": top_level_status,
        "warning_codes": warning_codes,
        "followup_resolution_escalation_action_plan_closure_cadence_counts": {
            "total_action_count": total_action_count,
            "open_action_count": open_action_count,
            "stale_open_action_count": stale_open_action_count,
            "net_new_closed_action_count_gl47": net_new_closed_action_count_gl47,
            "net_new_launch_gate_eligible_loop_count_gl47": net_new_launch_gate_eligible_loop_count_gl47,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_rows": cadence_rows,
    }


def _render_summary(report: dict[str, Any]) -> str:
    counts = report.get("followup_resolution_escalation_action_plan_closure_cadence_counts", {})
    ...
    if not isinstance(warning_codes, list):
        warning_codes = []
    rows = report.get("followup_resolution_escalation_action_plan_closure_cadence_rows", [])
    if not isinstance(rows, list):
        rows = []

    lines = [
        "# Real Trial Submission Queue Follow-Up Resolution Escalation Action Plan Closure Cadence Summary",
        "",
        "- GL-47 closure status: `%s`"
        % str(report.get("followup_resolution_escalation_action_plan_closure_status_gl47", "unknown")),
        "- GL-48 cadence status: `%s`"
        % str(report.get("followup_resolution_escalation_action_plan_closure_cadence_status", "unknown")),
        "- Cadence status: `%s`" % str(cadence.get("cadence_status", "unknown")),
    ...
    if rows:
        for row in rows:
            if not isinstance(row, dict):
                continue
            lines.append(
                "- `%s` state=%s cadence=%s modality=%s"
                % (
                    str(row.get("action_id", "")),
                    str(row.get("closure_state_gl47", "")),
...
def main() -> int:
    args = _parse_args()
    action_plan_closure_report_path = Path(
        str(args.submission_queue_followup_resolution_escalation_action_plan_closure_report).strip()
    ).resolve()
    ...
    try:
        ...
        if now_utc_text and not _is_utc_timestamp(now_utc_text):
            raise ValueError("--now-utc must be a timezone-aware UTC timestamp")
        now_utc = (
            _utc_timestamp_to_datetime(now_utc_text)
            if now_utc_text
        ...
        if not action_plan_closure_report_path.is_file():
            raise ValueError("GL-47 action-plan-closure report path does not exist: %s" % action_plan_closure_report_path)
        ...
        action_plan_closure_report = _read_json(action_plan_closure_report_path)
        collection_report = _read_json(collection_report_path)

        previous_report_path: Path | None = None
        previous_report: dict[str, Any] | None = None
        previous_arg = str(args.previous_action_plan_closure_cadence_report).strip()
        if previous_arg:
        ...
        report = _build_report(
            action_plan_closure_report=action_plan_closure_report,
            action_plan_closure_report_path=action_plan_closure_report_path,
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence summary written: %s"
            % summary_path
        )
    ...
    if not isinstance(counts, dict):
        counts = {}
    print(
        "Real trial follow-up resolution escalation action-plan closure cadence status=%s open_actions=%s stall_cycles=%s"
        % (
            str(report.get("followup_resolution_escalation_action_plan_closure_cadence_status", "unknown")),
    ...
    if bool(args.fail_on_overdue_stalled) and str(
        report.get("followup_resolution_escalation_action_plan_closure_cadence_status", "")
    ).strip().upper() == "ACTION_PLAN_CLOSURE_CADENCE_OVERDUE_STALLED":
        return 1
    return 0
