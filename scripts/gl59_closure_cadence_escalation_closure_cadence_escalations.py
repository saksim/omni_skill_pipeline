...
DEFAULT_ESCALATION_CLOSURE_CADENCE_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "current"
    / "status"
...
def _status_from_inputs(
    *,
    ...
    return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLEARED"


def _build_report(
    *,
    escalation_closure_cadence_report: dict[str, Any],
    ...
    cadence_warning_codes = escalation_closure_cadence_report.get("warning_codes", [])
    ...
    cadence_refresh = escalation_closure_cadence_report.get("refresh_cadence", {})
...
def main() -> int:
    ...
    try:
        ...
        report = _build_report(
            escalation_closure_cadence_report=escalation_closure_cadence_report,
            escalation_closure_cadence_report_path=escalation_closure_cadence_report_path,
            owner=owner,
            now_utc=now_utc,
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalation generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalations report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalations summary written: %s"
            % summary_path
        )
    ...
    print(
        "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalations status=%s open=%s overdue_stalled=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_status",
