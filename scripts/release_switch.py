...
def _run_stages(stage_specs: list[StageSpec], *, keep_going: bool = False, postgres_dsn: str = '') -> int:
    ...
    for stage in stage_specs:
        ...
        stage_env = _stage_environment(
            include_postgres_dsn=stage.name == 'release_gate',
...
def _release_gate_stage_contract_mismatches(
    ...
    for stage_name, expected_stages in expected.items():
        ...
        if command is None:
            ...
            continue
        if not _command_contains_script_token(command, 'scripts/linux_validate.py'):
            mismatches.append(
                {
                    'stage': stage_name,
                    'check': 'script',
                    'expected': 'scripts/linux_validate.py',
                    'actual': command,
                }
            )
...
def _release_gate_script_position_mismatches(
    ...
    expected_script = 'scripts/linux_validate.py'
    stage_names = ('beta_gate', 'ga_gate', 'roadmap_gate')
    mismatches: list[dict[str, Any]] = []
    for stage_name in stage_names:
        command = _plan_stage_command(release_gate_plan, stage_name)
...
def _evaluate_decision(args: argparse.Namespace) -> dict[str, Any]:
    ...
    gates = [
        ...
        },
        {
            ...
            'reason': (
                'release-gate stage commands target scripts/linux_validate.py and expected --stages packs'
        ...
        },
        {
            ...
            'reason': (
                'release-gate stage commands execute scripts/linux_validate.py as the first script token'
                if release_gate_script_position_pass and release_gate_script_position_check_enabled
                    ...
                    else 'release-gate stage commands do not execute scripts/linux_validate.py as the first script token'
                )
            ),
            'evidence': (
                [str(release_gate_path)]
        ...
        },
        {
            ...
            'reason': (
                'release-gate stage commands resolve scripts/linux_validate.py to repository canonical path'
