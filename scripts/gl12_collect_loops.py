...
def _build_collection_report(
    *,
    ...
    for loop in loops:
        ...
        if evidence_origin == "real":
            ...
            placeholder_fields = [
                ...
                if _contains_template_placeholder(loop.get(field_name))
            ]
    ...
    if real_evidence_template_placeholder_loop_count > 0:
        blockers.append("real_loop_template_placeholders_not_replaced")
...
def _render_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Real loops with template placeholders not replaced: `%s`"
        % str(alignment.get("real_evidence_template_placeholder_loop_count", 0)),
        "- Real template placeholder field count: `%s`"
        % str(alignment.get("real_evidence_template_placeholder_field_count", 0)),
