...
DEFAULT_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATIONS_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "current"
    / "status"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-60 escalation-closure diagnostics by comparing GL-59 escalation snapshots "
            "cycle-to-cycle and reconciling net-new closed GL-59 escalation items against GL-57 "
    ...
    parser.add_argument(
        ...
        default=str(DEFAULT_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATIONS_REPORT_PATH),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--previous-submission-queue-followup-resolution-escalation-action-plan-closure-cadence-escalation-acknowledgement-closure-cadence-escalation-closure-cadence-escalation-closure-cadence-escalation-closure-report",
        default="",
        help=(
            "Optional previous GL-60 report path. When omitted, script attempts to read existing "
    ...
    parser.add_argument(
        ...
        help='GL-60 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-60 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_STALLED."
        ),
    )
...
def _gl59_snapshot(report: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
...
def _previous_snapshot(previous_report: dict[str, Any] | None) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row in rows:
        if str(row.get("closure_item_status", "")).strip().lower() != "open":
            continue
        escalation_item_id = str(row.get("escalation_item_id_gl59", "")).strip()
...
def _gl57_net_new_closed_action_ids(
    ...
    previous_report_raw = str(
        input_paths.get(
            "previous_submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_report",
            "",
        )
    ).strip()
...
def _build_status(
    *,
    ...
    if not previous_available:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_BASELINE_INITIALIZED"

    status_gl59 = str(escalation_status_gl59).strip().upper()
    if (
        status_gl59
        ...
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_NOT_REQUIRED"
    if (
        net_new_closed_item_count > 0
        or net_new_open_item_count > 0
        or current_open_item_count != previous_open_item_count
    ):
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_PROGRESSING"
    if current_open_item_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_STALLED"
    return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CLEARED"
...
def _build_report(
    *,
    ...
    escalation_warnings_gl59 = escalation_report.get("warning_codes", [])
    if not isinstance(escalation_warnings_gl59, list):
        escalation_warnings_gl59 = []

    escalation_closure_status_gl57 = str(
        escalation_closure_report.get(
    ...
    escalation_closure_warnings_gl57 = escalation_closure_report.get("warning_codes", [])
    if not isinstance(escalation_closure_warnings_gl57, list):
        escalation_closure_warnings_gl57 = []

    current_open_rows, current_open_ids = _gl59_snapshot(escalation_report)
    ...
    gl57_net_new_closed_action_ids, gl57_net_new_closed_item_ids, gl57_previous_report_path = (
        _gl57_net_new_closed_action_ids(escalation_closure_report)
    )
    ...
    net_new_closed_backed_by_gl57_net_new_closed_item_ids: list[str] = []
    net_new_closed_without_gl57_net_new_closed_item_ids: list[str] = []
    for escalation_item_id in net_new_closed_item_ids:
        ...
        if action_id_gl48 and action_id_gl48 in gl57_net_new_closed_action_ids:
            net_new_closed_backed_by_gl57_net_new_closed_item_ids.append(escalation_item_id)
        else:
            net_new_closed_without_gl57_net_new_closed_item_ids.append(escalation_item_id)
    ...
    if previous_available:
        for row in current_open_rows:
            ...
            rows.append(
                {
                    "closure_item_id": "gl60-closure-cadence-escalation-closure-cadence-escalation-closure-%s"
                    % (
                        escalation_item_id
                        or str(row.get("action_id_gl48", "")).strip()
                        or str(row.get("closure_item_id_gl57", "")).strip()
                        or "unbound"
                    ),
                    "closure_item_status": "open",
                    "closure_progress_state": progress_state,
                    "owner": row_owner,
                    "escalation_item_id_gl59": escalation_item_id,
                    ...
                    "escalation_severity_gl59": str(row.get("escalation_severity", "")).strip().lower(),
                    "escalation_item_status_gl59": str(row.get("escalation_item_status", "")).strip().lower(),
                    "escalation_action_gl59": str(row.get("escalation_action", "")).strip(),
                    "cadence_item_status_gl58": str(row.get("cadence_item_status_gl58", "")).strip().lower(),
                    "next_refresh_due_utc_gl58": str(row.get("next_refresh_due_utc_gl58", "")).strip(),
                    "escalation_closure_status_gl57": escalation_closure_status_gl57,
                }
            )
        ...
        rows.sort(
            key=lambda item: (
                ...
                0 if str(item.get("escalation_severity_gl59", "")).strip() == "blocked_overdue_stalled" else 1,
                0 if str(item.get("escalation_severity_gl59", "")).strip() in {"due", "due_breached"} else 1,
                str(item.get("required_modality_gl47", "")),
                str(item.get("action_id_gl48", "")),
                str(item.get("escalation_item_id_gl59", "")),
            )
        )
    ...
    status = _build_status(
        escalation_status_gl59=escalation_status_gl59,
    ...
    if previous_available:
        if len(current_open_ids) > 0:
            warning_codes.append(
                "open_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_items_present"
            )
        if stale_open_item_count > 0:
            warning_codes.append(
                "acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_stalled"
            )
        if len(net_new_closed_without_gl57_net_new_closed_item_ids) > 0:
            warning_codes.append(
                "net_new_closed_escalation_items_without_gl57_escalation_closure_evidence"
            )
        warning_codes.extend(str(item).strip() for item in escalation_warnings_gl59 if str(item).strip())
        warning_codes.extend(
            str(item).strip() for item in escalation_closure_warnings_gl57 if str(item).strip()
        )
    ...
    if previous_available:
        counts = {
            ...
            "net_new_open_item_count": len(net_new_open_item_ids),
            "net_new_closed_item_count": len(net_new_closed_item_ids),
            "net_new_closed_backed_by_gl57_net_new_closed_item_count": len(
                net_new_closed_backed_by_gl57_net_new_closed_item_ids
            ),
            "net_new_closed_without_gl57_net_new_closed_item_count": len(
                net_new_closed_without_gl57_net_new_closed_item_ids
            ),
            "gl57_net_new_closed_item_count": len(gl57_net_new_closed_item_ids),
            "gl57_net_new_closed_action_item_count": len(gl57_net_new_closed_action_ids),
        }
    else:
        counts = {
            "total_item_count": 0,
    ...
    if not previous_available:
        owner_counts = {}
        carried_open_item_ids = []
        net_new_open_item_ids = []
        net_new_closed_item_ids = []
        net_new_closed_backed_by_gl57_net_new_closed_item_ids = []
        net_new_closed_without_gl57_net_new_closed_item_ids = []
        gl57_net_new_closed_item_ids = []
        rows = []
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalations_report": str(
                escalation_report_path
            ),
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_report": str(
                escalation_closure_report_path
            ),
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_previous_report": str(
                gl57_previous_report_path
            )
            if gl57_previous_report_path is not None
            else "",
            "previous_submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_report": str(
                previous_report_path
            )
            if previous_report_path is not None and previous_available
            else "",
        },
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_status_gl59": escalation_status_gl59,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_status_gl57": escalation_closure_status_gl57,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_counts": counts,
        "owner_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_counts": owner_counts,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_carried_open_item_ids": carried_open_item_ids,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_net_new_open_item_ids": net_new_open_item_ids,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_net_new_closed_item_ids": net_new_closed_item_ids,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_net_new_closed_backed_by_gl57_net_new_closed_item_ids": net_new_closed_backed_by_gl57_net_new_closed_item_ids,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_net_new_closed_without_gl57_net_new_closed_item_ids": net_new_closed_without_gl57_net_new_closed_item_ids,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_gl57_net_new_closed_item_ids": gl57_net_new_closed_item_ids,
...
def _render_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Real Trial Submission Queue Follow-Up Resolution Escalation Action Plan Closure Cadence Escalation Acknowledgement Closure Cadence Escalation Closure Cadence Escalation Closure Cadence Escalation Closure Summary",
        "",
        ...
        % str(
            report.get(
                "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_status_gl57",
                "unknown",
            )
        ),
        "- GL-60 escalation-closure status: `%s`"
        % str(
            report.get(
        ...
        "- Net-new closed backed by GL-57 closure evidence: `%s`"
        % str(_to_int(counts.get("net_new_closed_backed_by_gl57_net_new_closed_item_count", 0), default=0)),
        "",
        "## Warning Codes",
    ]
    if warning_codes:
    ...
    if rows:
        for row in rows:
            if not isinstance(row, dict):
                continue
            lines.append(
                "- `%s` state=%s severity_gl59=%s modality=%s action=%s"
                % (
                    str(row.get("closure_item_id", "")),
                    str(row.get("closure_progress_state", "")),
                    str(row.get("escalation_severity_gl59", "")),
...
def main() -> int:
    ...
    escalation_report_path = Path(
        str(
            args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalations_report
        ).strip()
    ).resolve()
    escalation_closure_report_path = Path(
        str(
            args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_report
        ).strip()
    ).resolve()
    ...
    try:
        if not escalation_report_path.is_file():
            raise ValueError("GL-59 escalation report path does not exist: %s" % escalation_report_path)
        if not escalation_closure_report_path.is_file():
            raise ValueError("GL-57 escalation-closure report path does not exist: %s" % escalation_closure_report_path)
        ...
        previous_arg = str(
            args.previous_submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_report
        ).strip()
        if previous_arg:
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure summary written: %s"
            % summary_path
        )
    ...
    if not isinstance(counts, dict):
        counts = {}
    print(
        "Real trial submission queue follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure status=%s open=%s carried_open=%s net_new_closed=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_status",
