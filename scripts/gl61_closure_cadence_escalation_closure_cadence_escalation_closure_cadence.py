...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-61 escalation-closure-cadence diagnostics by binding GL-60 "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional previous GL-61 report. When omitted, script attempts to read existing "
    ...
    parser.add_argument(
        ...
        help='GL-61 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-61 summary output path. Use "-" to skip writing file.',
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=24.0,
        help="Refresh cadence interval for GL-61 timing diagnostics (default: 24).",
    )
    parser.add_argument(
...
def _build_report(
    *,
    ...
    escalation_closure_status_gl60 = str(
    ...
    net_new_closed_item_count_gl60 = _to_int(
        escalation_closure_counts.get("net_new_closed_item_count", 0),
        default=0,
    )
    net_new_closed_backed_by_gl57_net_new_closed_item_count = _to_int(
        escalation_closure_counts.get("net_new_closed_backed_by_gl57_net_new_closed_item_count", 0),
        default=0,
    )

    not_required = (
        open_item_count <= 0
        and escalation_closure_status_gl60.upper()
        in {
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_NOT_REQUIRED",
            "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CLEARED",
        }
    )

    if (
        escalation_closure_status_gl60.upper()
    ...
    if cadence_status == "CADENCE_NOT_REQUIRED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_NOT_REQUIRED"
    elif cadence_status == "CADENCE_BASELINE_INITIALIZED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_BASELINE_INITIALIZED"
    elif cadence_status == "CADENCE_ON_SCHEDULE":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ON_SCHEDULE"
    elif cadence_status == "CADENCE_OVERDUE_STALLED":
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_OVERDUE_STALLED"
    else:
        top_level_status = "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_DUE"

    warning_codes: list[str] = []
    if open_item_count > 0:
        warning_codes.append(
            "open_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_items_present"
        )
    ...
    for row in escalation_closure_rows:
        if not isinstance(row, dict):
            continue
        closure_item_status_gl60 = str(row.get("closure_item_status", "")).strip().lower()
        if closure_item_status_gl60 != "open":
            continue

        closure_progress_state_gl60 = str(row.get("closure_progress_state", "")).strip().lower()
        if closure_progress_state_gl60 in {"carried_open", "net_new_open"}:
        ...
        elif closure_progress_state_gl60 == "closed_since_previous_cycle":
        ...
        cadence_rows.append(
            {
                "closure_item_id_gl60": str(row.get("closure_item_id", "")).strip(),
                "closure_progress_state_gl60": closure_progress_state_gl60,
                "closure_item_status_gl60": closure_item_status_gl60,
                "owner_gl60": str(row.get("owner", "")).strip() or owner,
                ...
                "next_refresh_due_utc_gl61": _datetime_to_utc_iso(next_refresh_due_dt),
            }
        )
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_report": str(
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_status_gl60": escalation_closure_status_gl60,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_status": top_level_status,
        "warning_codes": warning_codes,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_counts": {
            "total_item_count": total_item_count,
            "open_item_count": open_item_count,
            "stale_open_item_count": stale_open_item_count,
            "net_new_closed_item_count_gl60": net_new_closed_item_count_gl60,
            "net_new_closed_backed_by_gl57_net_new_closed_item_count": net_new_closed_backed_by_gl57_net_new_closed_item_count,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_rows": cadence_rows,
    }
...
def _render_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Real Trial Submission Queue Follow-Up Resolution Escalation Action Plan Closure Cadence Escalation Acknowledgement Closure Cadence Escalation Closure Cadence Escalation Closure Cadence Escalation Closure Cadence Summary",
        "",
    ...
    if rows:
        for row in rows:
            ...
            lines.append(
                ...
                % (
                    str(row.get("closure_item_id_gl60", "")),
                    str(row.get("closure_progress_state_gl60", "")),
...
def main() -> int:
    ...
    escalation_closure_report_path = Path(
        str(
            args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_report
        ).strip()
    ).resolve()
    ...
    try:
        ...
        if not escalation_closure_report_path.is_file():
            raise ValueError(
                "GL-60 escalation-closure report path does not exist: %s"
                % escalation_closure_report_path
            )
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalation closure cadence generation failed: %s"
            % exc,
            file=sys.stderr,
        )
        return 2
    ...
    if output_path is not None:
        ...
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalation closure cadence report written: %s"
            % output_path
        )
    if summary_path is not None:
        _write_text(summary_path, summary)
        print(
            "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalation closure cadence summary written: %s"
            % summary_path
        )
    ...
    print(
        "Real trial follow-up resolution escalation action-plan closure cadence escalation acknowledgement closure cadence escalation closure cadence escalation closure cadence escalation closure cadence status=%s open_items=%s stall_cycles=%s"
        % (
            str(
                report.get(
                    "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_escalation_closure_cadence_status",
