...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate GL-56 escalation diagnostics from GL-55 acknowledgement-closure-cadence-escalation-closure-cadence "
    ...
    parser.add_argument(
        ...
        help='GL-56 report output path. Use "-" to skip writing file.',
    )
    parser.add_argument(
        "--summary-output",
        default=str(DEFAULT_SUMMARY_PATH),
        help='GL-56 summary output path. Use "-" to skip writing file.',
    )
...
def _status_from_inputs(
    *,
    cadence_status_gl55: str,
    open_item_count: int,
    blocked_overdue_stalled_count: int,
    escalation_due_count: int,
) -> str:
    status = cadence_status_gl55.strip().upper()
    if (
        status
        == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_NOT_REQUIRED"
        and open_item_count <= 0
    ):
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_NOT_REQUIRED"
    if blocked_overdue_stalled_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_OVERDUE_STALLED"
    if escalation_due_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_DUE"
    if open_item_count > 0:
        return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_MONITORING"
    return "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_CLEARED"
...
def _build_report(
    *,
    ...
    for row in cadence_rows:
        ...
        row_owner = str(row.get("owner_gl54", "")).strip() or owner

        escalation_severity = "monitor"
        escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_monitoring"
        escalation_action = "continue_cadence_monitoring"

        if cadence_status_gl55.upper().endswith("_OVERDUE_STALLED"):
            escalation_severity = "blocked_overdue_stalled"
            escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_overdue_stalled"
            ...
            if overdue_by_hours >= escalate_after_due_hours:
                ...
                escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_due_breached"
                escalation_action = "escalate_due_breach"
            else:
                escalation_severity = "due"
                escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_due"
            ...
            escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_baseline_initialized"
            escalation_action = "start_first_refresh_cycle"
        elif cadence_item_status == "on_schedule":
            escalation_severity = "on_schedule"
            escalation_reason_code = "acknowledgement_closure_cadence_escalation_closure_cadence_on_schedule"
            escalation_action = "monitor_until_due"

        escalation_rows.append(
            {
                "escalation_item_id": "gl56-escalation-closure-cadence-escalation-%s"
                ...
                "owner": row_owner,
                "acknowledgement_ingestion_item_id_gl55": acknowledgement_ingestion_item_id,
                "action_id_gl48": action_id,
                "required_modality_gl47": str(row.get("required_modality_gl47", "")).strip().lower(),
                "cadence_item_status_gl55": cadence_item_status,
    ...
    escalation_rows.sort(
        key=lambda item: (
            ...
            str(item.get("acknowledgement_ingestion_item_id_gl55", "")),
        )
    )

    open_item_count = len(escalation_rows)
    status = _status_from_inputs(
        cadence_status_gl55=cadence_status_gl55,
    ...
    if blocked_overdue_stalled_count > 0:
        warning_codes.append("acknowledgement_closure_cadence_escalation_closure_cadence_overdue_stalled_escalation_required")
    if escalation_due_count > 0:
        warning_codes.append("acknowledgement_closure_cadence_escalation_closure_cadence_due_escalation_required")
    ...
    counts = {
        ...
        "cadence_stall_cycle_count_gl55": _to_int(cadence_counts.get("stall_cycle_count", 0), default=0),
        "cadence_overdue_stalled_cycles_threshold_gl55": _to_int(
    ...
    return {
        ...
        "owner": owner,
        "input_paths": {
            "submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_report": str(
                ack_closure_cadence_report_path
            ),
        },
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_status_gl55": cadence_status_gl55,
        ...
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_counts": counts,
        "owner_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_counts": owner_counts,
        "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_escalation_rows": escalation_rows,
    }
...
def _render_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "- GL-55 acknowledgement-closure-cadence-escalation-closure-cadence status: `%s`"
        % str(
            report.get(
                "followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_status_gl55",
                "unknown",
            )
        ),
        ...
        for row in rows:
            ...
            lines.append(
                ...
                % (
                    ...
                    str(row.get("cadence_item_status_gl55", "")),
...
def main() -> int:
    ...
    ack_closure_cadence_report_path = Path(
        str(
            args.submission_queue_followup_resolution_escalation_action_plan_closure_cadence_escalation_acknowledgement_closure_cadence_escalation_closure_cadence_report
        ).strip()
    ).resolve()
    ...
    try:
        if not ack_closure_cadence_report_path.is_file():
            raise ValueError("GL-55 acknowledgement-closure-cadence-escalation-closure-cadence report path does not exist: %s" % ack_closure_cadence_report_path)
    ...
    ).strip().upper() == "ACTION_PLAN_CLOSURE_CADENCE_ESCALATION_ACK_INGESTION_CLOSURE_CADENCE_ESCALATION_CLOSURE_CADENCE_ESCALATION_OVERDUE_STALLED":
        return 1
    return 0
