...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help='Calibration manifest path forwarded to scripts/tune_review_policy.py.',
    )
    parser.add_argument(
        '--calibration-report-output',
        default=str(DEFAULT_CALIBRATION_REPORT_OUTPUT),
        help='Calibration report output path forwarded to scripts/tune_review_policy.py.',
    )
    parser.add_argument(
        '--margin',
        type=float,
        default=0.03,
        help='Margin forwarded to scripts/tune_review_policy.py.',
    )
    parser.add_argument(
        '--fail-on-mismatch',
        action='store_true',
        help='Forward --fail-on-mismatch to scripts/tune_review_policy.py.',
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
